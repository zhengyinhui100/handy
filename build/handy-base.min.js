...
c,d){2==arguments.length&&(d=c,c=[]);n.require(c,function(){var c;if("function"==typeof d)try{c=d.apply(null,arguments),e.info("Loader define: "+b)}catch(f){e.error("Loader "+b+":factory define error:"+f.message,f);return}else c=d;a.namespace(b,c)})},require:function(a,g){for(var k="string"==typeof a?[a]:a,m=[],p=[],q=!0,r=0,w=k.length;r<w;r++){var l=k[r],t=f(l);t?p.push(t):(m.push(l),q&&(q=!1,s.push({deps:k,callback:g})),n.traceLog&&e.info("Resource not found: "+l))}if(0==m.length)return g&&g.apply(null,
p),0==p.length?p[0]:p;q=!1;k=0;for(p=m.length;k<p;k++)if(r=m[k],!u[r]){q=r;l=n.urlMap&&n.urlMap[q]&&n.urlMap[q].url;if(!l){w="";q=b.Object.alias(q);l=n.rootPath;if("string"==typeof l)w=l;else if("object"==typeof l)for(t in t=void 0,l)0==q.indexOf(t)&&(w=l[t],q=q.replace(t,""));else w="";l=/.css$/.test(q)?0==q.indexOf("/")?q:"/css/"+q:/.js$/.test(q)?q:q.replace(/\./g,"/")+".js";0!=l.indexOf("/")&&(l="/"+l);l=w+l}q=!0;u[r]={id:r,status:"loading"};w=c.bind(j,null,r);if(/.css$/.test(l))a:{r=l;l=h.getElementsByTagName("link");
...
j=0,k=document,h,m,s=k.documentMode,u="onhashchange"in window&&(void 0===s||7<s),n={delay:50,listen:function(e,m){if(!g)if(g=!0,n.listeners={},u)$(window).on("hashchange",function(){d(a.getHash())});else h=$('<iframe id="fff" tabindex="-1" style="display:none" width=0 height=0 title="empty" />').appendTo(k.body)[0],$(h).one("load",function(){b(a.getHash());setInterval(f,n.delay)});if(m in n.listeners)throw Error("Duplicate name");m=m||c.expando+ ++j;n.listeners[m]=e;return m},unListen:function(a){for(var b in n.listeners)(!a||
