...
handy.add("Loader",["B.Debug","B.Object","B.Function"],function(e,c,b,a){function d(a){var b={},g=[];"string"==typeof a&&(a=[a]);for(var d=0,f=a.length;d<f;d++){var h;h=a[d];h=/\.(css|js)$/.test(h)?w[h]&&"loaded"==w[h].status:s.urlMap&&s.urlMap[h]?s.urlMap[h].chkExist():c.ns(h);if(h)g.push(h);else return b.notExist=a[d],b}b.exist=g;return b}function f(a,b){function d(){d.isCalled||(d.isCalled=!0,clearTimeout(m),b())}if("SCRIPT"===a.nodeName){var c=a,f=d,h=function(){/loaded|complete|undefined/.test(c.readyState)&&
(c.onload=c.onerror=c.onreadystatechange=null,n.removeChild(c),c=null,setTimeout(f,0))};c.onload=c.onreadystatechange=h;c.onerror=function(){e.error(k+"load script error:\n"+c.src);h()}}else{var j=d;window.hasOwnProperty("attachEvent")?a.attachEvent("onload",j):setTimeout(function(){g(a,j)},0)}var m=setTimeout(function(){e.error("Time is out:"+a.src);d()},s.timeout)}function g(a,b){if(!b.isCalled){var d;if(l)a.sheet&&(d=!0);else if(a.sheet)try{a.sheet.cssRules&&(d=!0)}catch(c){if("SecurityError"===
c.name||"NS_ERROR_DOM_SECURITY_ERR"===c.name)d=!0}setTimeout(function(){d?b():g(a,b)},1)}}function h(a){s.showLoading(!1);t--;w[a].status="loaded";s.traceLog&&e.log(k+"Response:\n"+a);j()}function j(){for(var a=p.length-1;0<=a;a--){var b=p[a],g=d(b.deps);if(g.notExist)0==a&&0==t&&e.error(m+g.notExist);else{p.splice(a,1);b.callback.apply(null,g.exist);j();break}}}var k="[Handy Loader] ",m=k+"not found: ",n=document.head||document.getElementsByTagName("head")[0]||document.documentElement,l=navigator.userAgent.indexOf("AppleWebKit"),
p=[],t=0,w={},s={traceLog:!1,timeout:15E3,skinName:"skin",showLoading:function(){},define:function(b,g,d){b=a.Object.alias(b);2==arguments.length&&(d=g,g=[]);s.require(g,function(){var a;if("function"==typeof d)try{s.traceLog&&e.log(k+"define:\n"+b),a=d.apply(null,arguments)}catch(g){g.message=k+b+":\nfactory define error:\n"+g.message;e.error(g);return}else a=d;if(a){c.ns(b,a);var f=typeof a;if("object"==f||"function"==f)a.$ns=b}else e.error(k+"factory no return:\n"+b)})},require:function(g,c){for(var j=
"string"==typeof g?[g]:g,l=[],r=[],u=!0,x=0,z=j.length;x<z;x++){var q=j[x],q=a.Object.alias(q),v=d(q);v.notExist?(l.push(q),u&&(u=!1,p.push({deps:j,callback:c})),s.traceLog&&e.log(m+q)):r.push(v.exist[0])}if(0==l.length)return c&&c.apply(null,r),0==r.length?r[0]:r;u=!1;j=0;for(r=l.length;j<r;j++)if(x=l[j],!w[x]){u=x;q=s.urlMap&&s.urlMap[u]&&s.urlMap[u].url;if(!q){z="";q=s.rootPath;if("string"==typeof q)z=q;else if("object"==typeof q)for(v in v=void 0,q)0==u.indexOf(v)&&(z=q[v],u=u.replace(v,""));
else z="";q=/.css$/.test(u)?0==u.indexOf("/")?u:"/css/"+u:/.js$/.test(u)?u:u.replace(/\./g,"/")+".js";q=z.replace(/\/$/,"")+"/"+q.replace(/^\//,"")}u=!0;w[x]={id:x,status:"loading"};z=b.bind(h,null,x);s.traceLog&&e.log(k+"request:\n"+q);if(/.css$/.test(q))a:{x=q;q=n.getElementsByTagName("link");for(v=0;v<q.length;v++){var D=q[v].href;!s.skinNode&&0<=D.indexOf(s.skinName)&&(s.skinNode=q[v]);if(0<=D.indexOf(x)||0<=x.indexOf(D))break a}q=document.createElement("link");q.rel="stylesheet";q.href=x;f(q,
z);n.insertBefore(q,s.skinNode)}else x=q,q=document.createElement("script"),q.async="async",q.src=x,q.type="text/javascript",f(q,z),n.appendChild(q);t++}u&&s.showLoading(!0)}};return s});
...
handy.add("Template",["B.String","B.Debug"],function(e,c,b){function a(a){return j?"$r+="+a+";\n":"$r.push("+a+");\n"}function d(b){b=b.replace(/("|\\)/g,"\\$1").replace(/\r/g,"\\r").replace(/\n/g,"\\n");return a('"'+b+'"')}var f={openTag:"<%",closeTag:"%>",isEscape:!0,registerHelper:function(a,b){k[a]||(m+=a+"=oHelpers."+a+",");k[a]=b},tmpl:function(e,l){var p,t,w;if("string"==typeof e)p=e;else if(void 0!=e.length)p=e.join("");else if(p=e.tmpl,w=e.id)if(g[w])t=g[w];else if(!p){var s=document.getElementById(w);
s&&(p=(s.value||s.innerHTML).replace(/\s/g,""))}if(!t){if(!p){b.Debug.error("\u6a21\u677f\u672a\u5b9a\u4e49");return}a:{var y=m+"$r="+(j?'""':"[]")+";\n";b.Object.each(p.split(f.openTag),function(b,g){var c=g.split(f.closeTag);if(1==c.length)y+=d(c[0]);else{var e=y,j=c[0],j=j.replace(/this/g,"$data");0==j.indexOf("=")&&(j="("+j.replace(h,"")+")",j=a(j+'==undefined?"":'+j));y=e+(j+"\n");c[1]&&(y+=d(c[1]))}});y+="return "+(j?"$r;":'$r.join("");');try{var B=new Function("$data",y)}catch(C){c.log(y);
c.error(C);t=void 0;break a}B.$helpers=k;t=B}w&&(g[w]=t)}return l?t(l):t}},g={},h=/^=/,j="".trim,k={escape:e.escapeHTML,trim:e.trim},m="var oHelpers=arguments.callee.$helpers,escape=oHelpers.escape,trim=oHelpers.trim,";return f});
handy.add("HashChange",["handy.base.Debug","handy.base.Util"],function(e,c,b){function a(a){if(a==m)return!1;var b=k.contentWindow.document;b.open();b.write("<!doctype html><html><body>"+a+"</body></html>");b.close();m=a;e.log("set:"+k.contentWindow.document.body.innerText)}function d(){var b=c.getHash(),g=k.contentWindow.document.body.innerText;b!=m?(a(b),f(b)):g!=m&&(e.log("update:"+k.contentWindow.document.body.innerText),c.setHash(g))}function f(a){var b=p.listeners,g;for(g in b)b[g](a)}var g,
h=0,j=document,k,m,n=j.documentMode,l="onhashchange"in window&&(void 0===n||7<n),p={delay:50,listen:function(e,m){if(!g&&!g)if(g=!0,p.listeners={},l)$(window).on("hashchange",function(){f(c.getHash())});else k=$('<iframe id="fff" tabindex="-1" style="display:none" width=0 height=0 title="empty" />').appendTo(j.body)[0],$(k).one("load",function(){a(c.getHash());setInterval(d,p.delay)});if(m in p.listeners)$D.error(Error("Duplicate name"));else return m=m||b.expando+ ++h,p.listeners[m]=e,m},unListen:function(a){for(var b in p.listeners)(!a||
...
a?b.children:b.getEl().html()},setContent:function(a,b){if(b)return!b instanceof f&&(b=this.find(b)[0]),b.setContent(a);var c=this.getEl();c.contents().remove();if("string"==typeof a)c.html(a);else return this.add(a)},listen:function(a){if(!this._parseListenEvents("listen",a)){var b=a.name,c=a.context,d=a.times,f=a.target,e=a.custom||f||$H.contains(this._customEvents,b),l=a.handler;$H.isFunc(f)&&(f=f.call(this));if(e)a=$H.removeUndefined([f,b,l,c,d]),this[f?"listenTo":"on"].apply(this,a);else if(this.listened){var f=
this._listeners,d=a.el,e=a.method||"bind",p=a.selector,t=a.data,c=a.delegation=this._delegateHandler(l,c);$H.isFunc(d)&&(d=d.call(this));d=d?"string"==typeof d?this.findEl(d):d:this.getEl();if(p)if(t)d[e](p,b,t,c);else d[e](p,b,c);else if(t)d[e](b,t,c);else d[e](b,c);f.push(a)}else this.listeners.push(a)}},unlisten:function(a){if(!this._parseListenEvents("unlisten",a)){var b=a.name,c=a.handler;if(a.custom)this.off(b,c);else{var d=a.el,f="delegate"==a.method?"undelegate":"unbind";a=a.selector;for(var e,
d=d?"string"==typeof d?this.findEl(d):d:this.getEl(),l=this._listeners.length-1;0<=l;l--){var p=this._listeners[l];if(p.handler==c){e=p.delegation;this._listeners.splice(l,1);break}}if(a)d[f](a,b,e);else d[f](b,e)}}},initListeners:function(){if(this.listened)return!1;this.listened=!0;var a=this.listeners;this._listeners=[];for(var b=a.length-1;0<=b;b--)this.listen(a[b]);this.callChild()},clearListeners:function(){for(var a=this._listeners,b=a.length-1;0<=b;b--)this.unlisten(a[b]);this.off("all");
...
e;d++)if(n=a[d]=this.get(a[d]))delete this._byId[n.id],delete this._byId[n.cid],m=this.indexOf(n),this._models.splice(m,1),this.length--,b.silent||(b.index=m,n.trigger("remove",n,this,b)),this._removeReference(n,b);return c?a[0]:a},set:function(a,c){if(a){c=$H.extend({add:!0,remove:!0,merge:!0},c);c.parse&&(a=this.parse(a,c));var d=!$H.isArr(a),e=d?a?[a]:[]:$H.clone(a),k,m,n,l,p,t,w,s=c.at,y=this.model,B=this.comparator&&null==s&&!1!==c.sort,C="string"==typeof this.comparator?this.comparator:null,
A=[],r=[],u={},x=c.add,z=c.merge,q=c.remove,v=!B&&x&&q?[]:!1;k=0;for(m=e.length;k<m;k++){p=e[k]||{};n=p instanceof b?l=p:p[y.prototype.idAttribute||"id"];if(t=this.get(n))q&&(u[t.cid]=!0),z&&(p=p===l?l.attributes:p,c.parse&&(p=t.parse(p,c)),t.set(p,c),B&&(!w&&t.hasChanged(C))&&(w=!0)),e[k]=t;else if(x){c.id=n;l=e[k]=this._prepareModel(p,c);if(!l)continue;A.push(l);this._addReference(l,c)}l=t||l;v&&(l.isNew()||!u[l.id])&&v.push(l);u[l.id]=!0}if(q){k=0;for(m=this.length;k<m;++k)u[(l=this._models[k]).cid]||
r.push(l);r.length&&this.remove(r,c)}if(A.length||v&&v.length)if(B&&(w=!0),this.length+=A.length,null!=s){k=0;for(m=A.length;k<m;k++)this._models.splice(s+k,0,A[k])}else{v&&(this._models.length=0);n=v||A;k=0;for(m=n.length;k<m;k++)this._models.push(n[k])}w&&this.sort({silent:!0});if(!c.silent){k=0;for(m=A.length;k<m;k++)(l=A[k]).trigger("add",l,this,c);(w||v&&v.length)&&this.trigger("sort",this,c)}return d?e[0]:e}},each:function(a){$H.each(this._models,a)},reset:function(a,b){b||(b={});for(var c=
...
$Define("C.ModelList","C.AbstractComponent",function(e){e=e.define("ModelList");e.extend({emptyTips:"\u6682\u65e0\u7ed3\u679c",pdText:"\u4e0b\u62c9\u53ef\u5237\u65b0",pdComment:"\u4e0a\u6b21\u5237\u65b0\u65f6\u95f4\uff1a",tmpl:'<div class="hui-list s-scroll"><div class="hui-list-inner"><div<%if(this.children.length>0){%> style="display:none"<%}%> class="hui-list-empty js-empty"><%=this.emptyTips%></div><%if(this.hasPullRefresh){%><div class="hui-list-pulldown hui-pd-pull c-h-middle-container"><div class="c-h-middle"><span class="hui-icon hui-alt-icon hui-icon-arrow-d hui-light"></span><span class="hui-icon hui-alt-icon hui-icon-loading-mini"></span><div class="hui-pd-txt"><%if(this.pdText){%><div class="js-txt"><%=this.pdText%></div><%}%><%if(this.pdComment){%><div class="js-comment hui-pd-comment"><span class="js-pdComment"><%=this.pdComment%></span><span class="js-pdTime"><%=this.pdTime%></span></div><%}%></div></div></div><%}%><div class="js-item-container"><%=this.findHtml(">*")%></div><%if(this.hasPullRefresh){%><div class="hui-list-more"><a href="javascript:;" hidefocus="true" class="hui-btn hui-btn-gray hui-shadow hui-inline hui-radius-normal"><span class="hui-btn-txt">\u67e5\u770b\u66f4\u591a</span></a></div><%}%></div></div>',init:function(){var c=
this;c.itemXtype&&((c.defItem||(c.defItem={})).xtype=c.itemXtype);var b=c.model;b.each(function(a,b){c.addListItem(b)});c.listenTo(b,{add:function(a,b){c.addListItem(b)},remove:function(a,b){c.removeListItem(b)},reset:function(){c.removeListItem("emptyAll")}});c.hasPullRefresh=c.hasPullRefresh&&window.iScroll;c.hasPullRefresh&&(c.listen({name:"afterRender",handler:function(){var a=this,b=a.getEl();b.css({height:document.body.clientHeight-40});var c=b.find(".hui-list-pulldown"),e=c.find(".js-txt");
a.scroller=new window.iScroll(b[0],{useTransition:!0,topOffset:50,onRefresh:function(){c.hasClass("hui-pd-refresh")&&(c.removeClass("hui-pd-refresh hui-pd-release"),e.html("\u4e0b\u62c9\u53ef\u5237\u65b0"))},onScrollMove:function(){5<this.y&&!c.hasClass("hui-pd-release")?(c.addClass("hui-pd-release"),e.html("\u677e\u5f00\u53ef\u5237\u65b0"),this.minScrollY=0):5>this.y&&c.hasClass("hui-pd-release")&&(c.removeClass("hui-pd-release"),e.html("\u4e0b\u62c9\u53ef\u5237\u65b0"),this.minScrollY=-50)},onScrollEnd:function(){c.hasClass("hui-pd-release")&&
(c.addClass("hui-pd-refresh"),e.html("\u6b63\u5728\u5237\u65b0"),a.refresh())}})}}),c.listenTo(c.model,"sync",function(){c.findEl(".js-pdTime").html($H.formatDate($H.now(),"HH:mm"));setTimeout(function(){c.refreshScroller()},0)}),c.listen({name:"afterShow",handler:function(){c.refreshScroller()}}),c.listen({name:"click",method:"delegate",selector:".hui-list-more",handler:function(){c.getMore()}}))},addListItem:function(c){this.inited&&this.findEl(".js-empty").hide();this.add({model:c,renderTo:">.js-item-container"})},
removeListItem:function(c){"emptyAll"==c?this.remove(this.children):this.remove(function(b){return b.model&&b.model.id==c.id});0==this.children.length&&this.findEl(".js-empty").show()},refreshScroller:function(){this.scroller&&this.getEl()[0].clientHeight&&this.scroller.refresh()},destroy:function(){this.scroller&&(this.scroller.destroy(),this.scroller=null);this.callSuper()}});return e});
