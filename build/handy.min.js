...
handy.add("Json",function(){var d;d||(d={});(function(){function a(b){return 10>b?"0"+b:b}function c(b){f.lastIndex=0;return f.test(b)?'"'+b.replace(f,function(b){var e=k[b];return"string"===typeof e?e:"\\u"+("0000"+b.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+b+'"'}function e(b,f){var a,d,k,x,u=g,q,l=f[b];l&&("object"===typeof l&&"function"===typeof l.toJSON)&&(l=l.toJSON(b));"function"===typeof h&&(l=h.call(f,b,l));switch(typeof l){case "string":return c(l);case "number":return isFinite(l)?
...
typeof d.parse&&(d.parse=function(f,e){function c(b,f){var a,g,d=b[f];if(d&&"object"===typeof d)for(a in d)Object.prototype.hasOwnProperty.call(d,a)&&(g=c(d,a),void 0!==g?d[a]=g:delete d[a]);return e.call(b,f,d)}var a;f=String(f);b.lastIndex=0;b.test(f)&&(f=f.replace(b,function(b){return"\\u"+("0000"+b.charCodeAt(0).toString(16)).slice(-4)}));if(/^[\],:{}\s]*$/.test(f.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,"]").replace(/(?:^|:|,)(?:\s*\[)+/g,
"")))return a=eval("("+f+")"),"function"===typeof e?c({"":a},""):a;throw new SyntaxError("JSON.parse");})})();return d});
handy.add("Object",function(){function d(){var a=window.navigator.userAgent.match(/MSIE ([\d.]+)/);return a&&9>parseInt(a[0])?function(a,b,f){var c=[];if(a.length){b=b||0;for(f=f||a.length;b<f;b++)c.push(a[b])}return c}:function(a,b,f){return Array.prototype.slice.call(a,b||0,f||a.length)}}var a={_alias:{b:"handy.base",c:"handy.component",m:"handy.module"},namespace:function(c,e){var b=null,f,g,d,k;c=a.alias(c);g=c.split(".");b=g[0];b=(d=2==arguments.length)?eval("(function(){if (typeof "+b+' == "undefined"){'+
...
$Define("c.ControlGroup",["c.ComponentManager","c.AbstractComponent"],function(d,a){var c=a.define("ControlGroup");$HO.extend(c.prototype,{radius:"little",multi:!1,defItem:{xtype:"Button",extCls:"js-item",radius:null,shadow:!1,isInline:!1},tmpl:['<div class="hui-ctrlgp<%if(this.direction=="h"){%> hui-ctrlgp-h<%}else{%> hui-ctrlgp-v<%}%>">','<%=this.getHtml("$>*")%>',"</div>"],listeners:[{type:"click",selector:".js-item",method:"delegate",handler:function(a){var b=$(a.currentTarget),b=d.get(b.attr("id")).index();
this.onItemClick(a,b)}}],select:function(a){a="number"==typeof a?this.children[a]:this.find(a)[0];if(!this.multi&&!a.multi){var b=this.getSelected();b&&this.selectItem(b,!1);this.selectItem(a)}else this.selectItem(a,!a.selected)},getSelected:function(a){var b=[];this.each(function(c,d){d.selected&&b.push(a?c:d)});return 0<b.length?this.multi?b:b[0]:null},selectItem:function(a,b){(b=!1!=b)?(a.selected=b,a.select?a.select():a.active()):(a.selected=b,a.select?a.select(b):a.unactive())},val:function(a){if(a){var b=
a.split(",");this.each(function(a,c){c.select($HO.contains(b,c.value))})}else return a=this.find("$>[selected=true]"),b=[],$HO.each(a,function(a,c){b.push(c.value)}),b.join(",")},onItemClick:function(a,b){this.select(b);this.itemClick&&this.itemClick(this.children[b],b)}});return c});$Define("c.Icon","c.AbstractComponent",function(d){d=d.define("Icon");$HO.extend(d.prototype,{hasBg:!0,tmpl:['<span class="hui-icon hui-icon-<%=this.name%><%if(this.hasBg){%> hui-icon-bg<%}%>"></span>']});return d});
...
parseItem:function(a){"Icon"==a.xtype&&(this.hasIcon=!0)}});return d});
...
a=!1!=a;var c=this.find("input"),e=this.getEl();a?(c.attr("checked",!0),e.addClass("hui-radio-on")):(c.removeAttr("checked"),e.removeClass("hui-radio-on"))},val:function(a){if(a)this.value=a,this.find("input").val(a);else return this.value}});return d});
...
a=!1!=a;var c=this.find("input"),e=this.getEl();a?(c.attr("checked",!0),e.addClass("hui-chkbox-on")):(c.removeAttr("checked"),e.removeClass("hui-chkbox-on"))},val:function(a){if(a)this.value=a,this.find("input").val(a);else return this.value}});return d});
...
"<%=this.children[i].content%>","</div>","<%}%>","</div>"],parseItem:function(a){a.selected&&(a.isActive=!0)},onItemClick:function(a,b){this.find(".js-tab-content").hide().eq(b).show();this.callSuper([a,b])}});return c});
...
b.cancelCall())&&b.hide()}}))}});return c});
$Define("c.Menu",["c.AbstractComponent","c.Popup","c.ControlGroup"],function(d,a,c){d=d.define("Menu",a);$HO.extend(d.prototype,c.prototype,{notCover:!0});$HO.extend(d.prototype,{tmpl:['<div class="hui-menu<%if(this.markType){%> hui-menu-mark<%}%>">',"<ul>","<%for(var i=0,len=this.children.length;i<len;i++){%>",'<li class="hui-menu-item">',"<%=this.children[i].getHtml()%>",'<%if(this.markType){%><span class="hui-icon-mark"></span><%}%>',"</li>","<%}%>","</ul>","</div>"],selectItem:function(a,b){b=
!1!=b;"dot"==this.markType?(a.selected=b,a.getEl().parent()[!1==b?"removeClass":"addClass"]("hui-item-mark")):"active"==this.markType?c.prototype.selectItem.call(this,a,b):a.selected=b}});return d});
...
j.append(g);$HL.fire("afterRender",g);d.render(g);e.currentMod==b&&e._showMod(d);d.afterRender()})},_showMod:function(a){if(this.navigator&&this.navigator.navigate(a,this))return!1;this._hideAll();a._container.show();a.isActive=!0},_hideAll:function(){var a=this.modules,d;for(d in a)a[d]._container.hide(),a[d].isActive=!1},initialize:function(a){a&&(this.conf=a,$HO.extend(this,a),this.container=a.container?$(a.container):$(document.body));this.defModPackage+=".";this.history=new d;this.modules={}},
go:function(a,d){var b=a.modName,f=this.currentMod,g=this.modules,j=g[f];if(f==b)j.waiting||j.reset();else{if(j&&!j.waiting&&!j.exit())return!1;(f=g[b])?f.useCache?(this._showMod(f),f.cache(a)):f.waiting||(f.destroy(),this._createMod(a)):this._createMod(a);!0!=d&&this.history.saveState({onStateChange:$H.Function.bind(this.go,this),param:a});this.currentMod=b}}});return a});
