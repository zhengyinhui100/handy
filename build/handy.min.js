...
else z="";q=/.css$/.test(u)?0==u.indexOf("/")?u:"/css/"+u:/.js$/.test(u)?u:u.replace(/\./g,"/")+".js";0!=q.indexOf("/")&&(q="/"+q);q=z+q}u=!0;w[x]={id:x,status:"loading"};z=b.bind(h,null,x);s.traceLog&&e.info(k+"request:\n"+q);if(/.css$/.test(q))a:{x=q;q=n.getElementsByTagName("link");for(v=0;v<q.length;v++){var D=q[v].href;!s.skinNode&&0<=D.indexOf(s.skinName)&&(s.skinNode=q[v]);if(0<=D.indexOf(x)||0<=x.indexOf(D))break a}q=document.createElement("link");q.rel="stylesheet";q.href=x;f(q,z);n.insertBefore(q,
s.skinNode)}else x=q,q=document.createElement("script"),q.async="async",q.src=x,q.type="text/javascript",f(q,z),n.appendChild(q);t++}u&&s.showLoading(!0)}};return s});
...
html:function(a){return(new this($H.extend({autoRender:!1},a))).getHtml()}});return f});
...
$Define("C.ModelList","C.AbstractComponent",function(e){e=e.define("ModelList");e.extend({emptyTips:"\u6682\u65e0\u7ed3\u679c",tmpl:['<div class="hui-list">','<%=this.findHtml(">*")%>','<div id="emptyContent"<%if(this.children.length>0){%> style="display:none"<%}%> class="hui-list-empty"><%=this.emptyTips%></div>',"</div>"],init:function(){var c=this;c.itemXtype&&((c.defItem||(c.defItem={})).xtype=c.itemXtype);var b=c.model;b.each(function(a,b){c.addListItem(b)});c.listenTo(b,{add:function(a,b){c.addListItem(b)},
remove:function(a,b){c.removeListItem(b)},reset:function(){c.removeListItem("emptyAll")}});console.log(c)},addListItem:function(c){this.inited&&this.findEl("#emptyContent").hide();this.add({model:c})},removeListItem:function(c){"emptyAll"==c?this.remove(this.children):this.remove(function(b){return b.model&&b.model.id==c.id});0==this.children.length&&this.findEl("#emptyContent").show()}});return e});
