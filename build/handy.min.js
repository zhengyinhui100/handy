...
handy.add("Json",function(){function e(a){g.lastIndex=0;g.test(a)&&(a=a.replace(g,function(a){var d=h[a];return"string"===typeof d?d:"\\u"+("0000"+a.charCodeAt(0).toString(16)).slice(-4)}));return'"'+a+'"'}function c(g,b){var h,j,k,m,n=d,l,s=b[g];s&&("object"===typeof s&&"function"===typeof s.toJSON)&&(s=s.toJSON(g));"function"===typeof a&&(s=a.call(b,g,s));switch(typeof s){case "string":return e(s);case "number":return isFinite(s)?String(s):"null";case "boolean":case "null":return String(s);case "object":if(!s)return"null";
d+=f;l=[];if("[object Array]"===Object.prototype.toString.apply(s)){m=s.length;for(h=0;h<m;h+=1)l[h]=c(h,s)||"null";k=0===l.length?"[]":d?"[\n"+d+l.join(",\n"+d)+"\n"+n+"]":"["+l.join(",")+"]";d=n;return k}if(a&&"object"===typeof a){m=a.length;for(h=0;h<m;h+=1)"string"===typeof a[h]&&(j=a[h],(k=c(j,s))&&l.push(e(j)+(d?": ":":")+k))}else for(j in s)Object.prototype.hasOwnProperty.call(s,j)&&(k=c(j,s))&&l.push(e(j)+(d?": ":":")+k);k=0===l.length?"{}":d?"{\n"+d+l.join(",\n"+d)+"\n"+n+"}":"{"+l.join(",")+
...
handy.add("Loader",["B.Debug","B.Object","B.Function"],function(e,c,b,a){function d(a){var b={},d=[];"string"==typeof a&&(a=[a]);for(var g=0,f=a.length;g<f;g++){var h;h=a[g];h=/\.(css|js)$/.test(h)?x[h]&&"loaded"==x[h].status:r.urlMap&&r.urlMap[h]?r.urlMap[h].chkExist():c.ns(h);if(h)d.push(h);else return b.notExist=a[g],b}b.exist=d;return b}function f(a,b){function d(){d.isCalled||(d.isCalled=!0,clearTimeout(j),b())}if("SCRIPT"===a.nodeName){var c=a,f=d;c.onload=c.onerror=c.onreadystatechange=function(){/loaded|complete|undefined/.test(c.readyState)&&
(c.onload=c.onerror=c.onreadystatechange=null,n.removeChild(c),c=null,setTimeout(f,0))}}else{var h=d;window.hasOwnProperty("attachEvent")?a.attachEvent("onload",h):setTimeout(function(){g(a,h)},0)}var j=setTimeout(function(){e.error("Time is out:"+a.src);d()},r.timeout)}function g(a,b){if(!b.isCalled){var d;if(l)a.sheet&&(d=!0);else if(a.sheet)try{a.sheet.cssRules&&(d=!0)}catch(c){if("SecurityError"===c.name||"NS_ERROR_DOM_SECURITY_ERR"===c.name)d=!0}setTimeout(function(){d?b():g(a,b)},1)}}function h(a){r.showLoading(!1);
t--;x[a].status="loaded";r.traceLog&&e.info(k+"Response: "+a);j()}function j(){for(var a=q.length-1;0<=a;a--){var b=q[a],g=d(b.deps);if(g.notExist)0==a&&0==t&&$D.error(m+g.notExist);else{q.splice(a,1);b.callback.apply(null,g.exist);j();break}}}var k="[Handy Loader] ",m=k+"not found: ",n=document.head||document.getElementsByTagName("head")[0]||document.documentElement,l=navigator.userAgent.indexOf("AppleWebKit"),q=[],t=0,x={},r={traceLog:!1,timeout:15E3,skinName:"skin",showLoading:function(){},define:function(b,
d,g){b=a.Object.alias(b);2==arguments.length&&(g=d,d=[]);r.require(d,function(){var a;if("function"==typeof g)try{a=g.apply(null,arguments),r.traceLog&&e.info(k+"define: "+b)}catch(d){d.message=k+b+":factory define error:"+d.message;e.error(d);return}else a=g;if(a){c.ns(b,a);var f=typeof a;if("object"==f||"function"==f)a.$ns=b}else $D.error(k+"factory no return: "+b)})},require:function(g,c){for(var j="string"==typeof g?[g]:g,l=[],s=[],u=!0,w=0,y=j.length;w<y;w++){var p=j[w],p=a.Object.alias(p),v=
d(p);v.notExist?(l.push(p),u&&(u=!1,q.push({deps:j,callback:c})),r.traceLog&&e.info(m+p)):s.push(v.exist[0])}if(0==l.length)return c&&c.apply(null,s),0==s.length?s[0]:s;u=!1;j=0;for(s=l.length;j<s;j++)if(w=l[j],!x[w]){u=w;p=r.urlMap&&r.urlMap[u]&&r.urlMap[u].url;if(!p){y="";p=r.rootPath;if("string"==typeof p)y=p;else if("object"==typeof p)for(v in v=void 0,p)0==u.indexOf(v)&&(y=p[v],u=u.replace(v,""));else y="";p=/.css$/.test(u)?0==u.indexOf("/")?u:"/css/"+u:/.js$/.test(u)?u:u.replace(/\./g,"/")+
".js";0!=p.indexOf("/")&&(p="/"+p);p=y+p}u=!0;x[w]={id:w,status:"loading"};y=b.bind(h,null,w);if(/.css$/.test(p))a:{w=p;p=n.getElementsByTagName("link");for(v=0;v<p.length;v++){var B=p[v].href;!r.skinNode&&0<=B.indexOf(r.skinName)&&(r.skinNode=p[v]);if(0<=B.indexOf(w)||0<=w.indexOf(B))break a}p=document.createElement("link");p.rel="stylesheet";p.href=w;f(p,y);n.insertBefore(p,r.skinNode);r.traceLog&&e.info(k+"request:"+w)}else w=p,p=document.createElement("script"),p.async="async",p.src=w,p.type=
"text/javascript",f(p,y),n.appendChild(p),r.traceLog&&e.info(k+"request:"+w);t++}u&&r.showLoading(!0)}};return r});
...
handy.add("Template","B.String",function(e,c){function b(a){return h?"$r+="+a+";\n":"$r.push("+a+");\n"}function a(a){a=a.replace(/("|\\)/g,"\\$1").replace(/\r/g,"\\r").replace(/\n/g,"\\n");return b('"'+a+'"')}var d={openTag:"<%",closeTag:"%>",isEscape:!0,registerHelper:function(a,b){j[a]||(k+=a+"=oHelpers."+a+",");j[a]=b},tmpl:function(e,n){var l,q,t;if("string"==typeof e)l=e;else if(void 0!=e.length)l=e.join("");else if(l=e.tmpl,t=e.id)if(f[t])q=f[t];else if(!l){var x=document.getElementById(t);
x&&(l=(x.value||x.innerHTML).replace(/\s/g,""))}if(!q){if(!l){c.Debug.error("\u6a21\u677f\u672a\u5b9a\u4e49");return}var r=k+"$r="+(h?'""':"[]")+";\n";c.Object.each(l.split(d.openTag),function(c,f){var h=f.split(d.closeTag);if(1==h.length)r+=a(h[0]);else{var e=r,j=h[0],j=j.replace(/this/g,"$data");0==j.indexOf("=")&&(j="("+j.replace(g,"")+")",j=b(j+'==undefined?"":'+j));r=e+(j+"\n");h[1]&&(r+=a(h[1]))}});r+="return "+(h?"$r;":'$r.join("");');l=new Function("$data",r);l.$helpers=j;q=l;t&&(f[t]=q)}return n?
q(n):q}},f={},g=/^=/,h="".trim,j={escape:e.escapeHTML,trim:e.trim},k="var oHelpers=arguments.callee.$helpers,escape=oHelpers.escape,trim=oHelpers.trim,";return d});
...
[b[c]].concat(d));return!0}return!1},_parseListenEvents:function(a,b){var d=this;return d._parseEvents(b.name,function(c){b.name=c[0];2==c.length&&(b.handler=c[0]);d[a].call(d,b)})},initialize:function(a){if(!this.inited){this.manager=this.constructor.manager||$H.getSingleton(e);var b=this.tmpl;$H.isFunc(b)||(this.tmpl=this.constructor.prototype.tmpl=$H.tmpl(b));this.doConfig(a);this.parseItems();!1!=this.autoRender&&this.render();this.manager.register(this);this.inited=!0}},doConfig:function(a){var b=
...
typeof g?g:g.$ns),!0}});b.renderTo=d.renderTo?$(d.renderTo):$(document.body)},getEl:function(){return this._container},getId:function(){return this._id||(this._id=this.manager.generateId(this.cid))},initHtml:function(){return this.tmpl(this)},getHtml:function(){var g=this.getId(),c;c="visibility"==this.displayMode?"visibility:hidden;":"display:none;";var f=this.initHtml(),e=a.test(f),m="js-"+this.manager.type+" js-"+this.xtype+" "+this.extCls+" ";e&&(f=f.replace(d,"$1"+m));return f=f.replace(b,'$1 id="'+
...
this;b.parent;)if(b=b.parent,a&&this.match(a,b))return b;return a||b===this?null:b},index:function(){var a=this,b=a.parent;if(b){var d;b.each(function(b,c){if(c==a)return d=b,!1});return d}return null},callChild:function(a,b){0!=this.children.length&&(a&&"string"!=typeof a&&(b=a,a=null),a=a||arguments.callee.caller.$name,this.each(function(d,c){b?c[a].apply(c,b):c[a].call(c)}))},add:function(a,b){if(!this._applyArray()){var d=void 0==b;if(this.startParseItems){if(a instanceof f)a.parent=this;else{this.defItem&&
$H.extend(a,this.defItem,{notCover:!0});this.parseItem(a);var c=this.manager.getClass(a.xtype);c?(a.renderTo||(this.inited?a.renderTo=this.getEl():a.autoRender=!1),a.parent=this,a=new c(a)):$D.error("xtype:"+a.xtype+"\u672a\u627e\u5230")}c=this.children;d?c.push(a):c.splice(b,0,a);this.trigger("add",a);return a}c=this.items;d?c.push(a):c.splice(b,0,a)}},remove:function(a){if(!this._applyArray()){var b=this.children,d=!1,c;if($H.isNum(a))c=a,a=b[c];else if($H.isStr(a)||$H.isFunc(a)){a=this.find(a);
...
this))}},initialize:function(b,a){this.dao=this.dao||$H.getSingleton(e);this._initDepFields();var d=b||{};a||(a={});this.cid=$H.Util.getUuid();this._attributes={};a.collection&&(this.collection=a.collection);a.parse&&(d=this.parse(d,a)||{});d=$H.extend(d,$H.Util.result(this,"defaults"),{notCover:!0});this.set(d,a);this._changed={};$S.push(this)},toJSON:function(){return $H.clone(this._attributes)},sync:function(b,a,d){return this.dao.sync(b,a,d)},get:function(b){return this._attributes[b]},escape:function(b){return $H.String.escapeHTML(this.get(b))},
has:function(b){b=this.get(b);return null!=b&&void 0!=b},set:function(b,a,d){var c;"object"===typeof b?(c=b,d=a):(c={})[b]=a;d||(d={});c=this._parseFields(c);if(!this._validate(c,d))return!1;var g=d.unset;b=d.silent;var e=[],j=this._changing;this._changing=!0;j||(this._previousAttributes=$H.Object.clone(this._attributes),this._changed={});var k=this._attributes,m=this._previousAttributes;this.idAttribute in c&&(this.id=c[this.idAttribute]);for(var n in c)a=c[n],$H.equals(k[n],a)||e.push(n),$H.equals(m[n],
a)?delete this._changed[n]:this._changed[n]=a,g?delete k[n]:k[n]=a;if(!b){e.length&&(this._pending=d);a=0;for(c=e.length;a<c;a++)this.trigger("change:"+e[a],this,k[e[a]],d)}if(j)return this;if(!b)for(;this._pending;)d=this._pending,this._pending=!1,this.trigger("change",this,d);this._changing=this._pending=!1;return this},unset:function(b,a){a=a||{};a.unset=!0;return this.set(b,void 0,a)},clear:function(b){var a={},c;for(c in this._attributes)a[c]=void 0;b=b||{};b.unset=!0;return this.set(a,b)},hasChanged:function(b){var a=
...
this.get(a[d]))delete this._byId[n.id],delete this._byId[n.cid],m=this.indexOf(n),this._models.splice(m,1),this.length--,b.silent||(b.index=m,n.trigger("remove",n,this,b)),this._removeReference(n,b);return c?a[0]:a},set:function(a,c){c=$H.extend({add:!0,remove:!0,merge:!0},c);c.parse&&(a=this.parse(a,c));var d=!$H.isArr(a),e=d?a?[a]:[]:$H.clone(a),k,m,n,l,q,t,x,r=c.at,C=this.model,A=this.comparator&&null==r&&!1!==c.sort,D="string"==typeof this.comparator?this.comparator:null,z=[],s=[],u={},w=c.add,
y=c.merge,p=c.remove,v=!A&&w&&p?[]:!1;k=0;for(m=e.length;k<m;k++){q=e[k]||{};n=q instanceof b?l=q:q[C.prototype.idAttribute||"id"];if(t=this.get(n))p&&(u[t.cid]=!0),y&&(q=q===l?l.attributes:q,c.parse&&(q=t.parse(q,c)),t.set(q,c),A&&(!x&&t.hasChanged(D))&&(x=!0)),e[k]=t;else if(w){c.id=n;l=e[k]=this._prepareModel(q,c);if(!l)continue;z.push(l);this._addReference(l,c)}l=t||l;v&&(l.isNew()||!u[l.id])&&v.push(l);u[l.id]=!0}if(p){k=0;for(m=this.length;k<m;++k)u[(l=this._models[k]).cid]||s.push(l);s.length&&
this.remove(s,c)}if(z.length||v&&v.length)if(A&&(x=!0),this.length+=z.length,null!=r){k=0;for(m=z.length;k<m;k++)this._models.splice(r+k,0,z[k])}else{v&&(this._models.length=0);n=v||z;k=0;for(m=n.length;k<m;k++)this._models.push(n[k])}x&&this.sort({silent:!0});if(!c.silent){k=0;for(m=z.length;k<m;k++)(l=z[k]).trigger("add",l,this,c);(x||v&&v.length)&&this.trigger("sort",this,c)}return d?e[0]:e},reset:function(a,b){b||(b={});for(var c=0,d=this._models.length;c<d;c++)this._removeReference(this._models[c],
...
$Define("C.Icon","C.AbstractComponent",function(e){e=e.define("Icon");e.extend({tmpl:'<span class=";<%if(this.isAlt){%>; hui-alt-icon;<%}%>; hui-icon-<%=this.name%>;<%if(!this.noBg){%>; hui-icon-bg;<%}%>"></span>'.split(";")});return e});
...
"</div>"],selectItem:function(a,c){c=!1!=c;"dot"==this.markType?(a.selected=c,a.getEl().parent()[!1==c?"removeClass":"addClass"]("hui-item-mark")):"active"==this.markType?b.prototype.selectItem.call(this,a,c):a.selected=c}});return e});$Define("C.List",["C.AbstractComponent","C.ControlGroup"],function(e,c){var b=e.define("List",c);b.extend({cls:"list",tmpl:["<div>",'<div class="hui-list-item c-clear">',"</div>","</div>"]});return b});
