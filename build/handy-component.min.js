...
a=this.content;void 0==a&&!b.items&&(a="");$H.isSimple(a)&&(a={text:a,underline:!0});a&&(this.noPadding=!0,this.add(a))}});return a});$Define("C.Form","C.AbstractComponent",function(a){a=a.define("Form");a.extend({tmpl:'<div>;<form action="">;<div class="hui-form-tips c-error"></div>;<%=this.findHtml(">*")%>;</form>;</div>'.split(";")});return a});
...
d(a)}})}});c.extend({okTxt:"\u786e\u5b9a",cancelTxt:"\u53d6\u6d88",clickHide:!1,radius:"little",tmpl:'<div>;<%=this.findHtml(">[xrole=dialog-header]")%>;<div class="hui-dialog-body">;<%if(this.content){%><%=this.content%><%}else{%>;<div class="hui-body-content">;<h1 class="hui-content-title"><%=this.contentTitle%></h1>;<div class="hui-content-msg"><%=this.contentMsg%></div>;<%=this.findHtml(">[xrole=dialog-content]")%>;</div>;<%}%>;<%if(!this.noAction){%>;<div class="hui-body-action">;<%=this.findHtml(">[xrole=dialog-action]")%>;</div>;<%}%>;</div>;</div>'.split(";"),
...
$Define("C.ModelList","C.AbstractComponent",function(a){a=a.define("ModelList");a.extend({emptyTips:"\u7a7a",tmpl:['<div class="hui-list">','<div id="emptyContent"<%if(this.children.length>0){%> style="display:none"<%}%> class="hui-list-empty"><%=this.emptyTips%></div>',"</div>"],init:function(){var a=this;a.listenTo(a.model,{add:function(c,f){a.addListItem(f)},remove:function(c,f){a.removeListItem(f)},reset:function(){a.removeListItem("emptyAll")}})},addListItem:function(a){this.inited&&this.findEl("#emptyContent").hide();
this.add({model:a})},removeListItem:function(a){"emptyAll"==a?this.remove(this.children):this.remove(function(c){return c.model&&c.model.id==a.id});0==this.children.length&&this.findEl("#emptyContent").show()}});return a});
