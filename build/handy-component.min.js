...
$Define("C.ComponentManager",["CM.AbstractManager","CM.ViewManager"],function(a,b){var c=a.derive({type:"component",initialize:function(){var f=$H.getSingleton(b);this._types=f._types;this._all=f._all}});$CM=new c;return c});
$Define("C.AbstractComponent",["CM.ViewManager","CM.View"],function(a,b){$C=$H.ns("C",{});var c=$H.createClass();$H.inherit(c,b,{xtype:"AbstractComponent",activeCls:"hui-active",hasConfig:function(b,c){if(!c)return!1;if($H.isArray(c))for(var a=0,e=c.length;a<e;a++){if(this.match(b,c[a]))return!0}else if(this.match(b,c))return!0;return!1},doConfig:function(){this.callSuper();this.cls||(this.cls=this.xtype.toLowerCase());this.extCls=this.getExtCls();this.icon&&this.add({xtype:"Icon",name:this.icon});
...
b.push("hui-focus");this.isInline&&b.push("hui-inline");return 0<b.length?b.join(" "):""},active:function(){this.getEl().addClass(this.activeCls)},unactive:function(){this.getEl().removeClass(this.activeCls)},txt:function(b){var c=this.findEl(".js-"+this.cls+"-txt");0==c.length&&(c=this.findEl(".hui-"+this.cls+"-txt"));if(void 0!=b)c.text(b);else return c.text()},valid:function(){var b=this.validator;if(b){var c=this.val();b.error||(b.error=function(b){new $C.Tips({text:b,theme:"error"})});return b=
$H.Validator.valid(c,b)}for(var c=this.children,a=0,e=c.length;a<e;a++)if(b=c[a].valid(),!b)return b;return!0}},{define:function(b,g){var d=$H.createClass();$H.inherit(d,g||c,null,null,{notCover:function(b){return"define"==b}});$H.getSingleton(a).registerType(b,d);return $C[b]=d}});return c});$Define("C.Panel","C.AbstractComponent",function(a){a=a.define("Panel");a.extend({tmpl:['<div><%=this.content||this.findHtml(">*")%></div>']});return a});
...
"disabled")}}),this.listeners.push({name:"hide",custom:!0,handler:function(){!0!=$H.Events.trigger("component.popup.hide")&&setTimeout(function(){$("input,textarea,select").removeAttr("disabled")},500)}}))},afterShow:function(){var c=this,a=c.getEl();a.css("z-index",1E3*b+1E3);if(!c.width&&c.parent){var d=c.width=c.parent.getEl().outerWidth();a.css("width",d)}a=c.showPos;if("string"==typeof a)c[a]();else"function"==typeof a&&a.call(c);c.noMask||c.mask();c.timeout&&setTimeout(function(){c.destroyed||
...
"</div>"],listeners:[{name:"click",handler:function(){this.showOptions()}}],doConfig:function(b){var c=this;c.callSuper();for(var a=b.options,g=!1,d=0,e=a.length;d<e;d++){var h=a[d];if(h.value==b.value){c.text=h.text;g=h.selected=!0;break}}g||delete c.value;c.add({itemClick:function(b){c.val(b.value)},width:c.width,items:a})},showOptions:function(){this.children[0].show()},val:function(b){if(b){if(this.value!=b){var c=this.children[0],a=c.find('>[value="'+b+'"]');0<a.length&&(this.trigger("change"),
...
d(b)}})}});c.extend({okTxt:"\u786e\u5b9a",cancelTxt:"\u53d6\u6d88",clickHide:!1,radius:"little",tmpl:'<div class="hui-dialog">;<%=this.findHtml(">[xrole=\'dialog-header\']")%>;<div class="hui-dialog-body">;<%if(this.content){%><%=this.content%><%}else{%>;<div class="hui-body-content">;<h1 class="hui-content-title"><%=this.contentTitle%></h1>;<div class="hui-content-msg"><%=this.contentMsg%></div>;<%=this.findHtml(">[xrole=\'dialog-content\']")%>;</div>;<%}%>;<%if(!this.noAction){%>;<div class="hui-body-action">;<%=this.findHtml(">[xrole=\'dialog-action\']")%>;</div>;<%}%>;</div>;</div>'.split(";"),
