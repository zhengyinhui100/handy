...
$Define("C.AbstractComponent",["CM.ViewManager","CM.View"],function(a,b){$C=$H.ns("C",{});var c=$H.createClass();$H.inherit(c,b,{xtype:"AbstractComponent",activeCls:"hui-active",hasConfig:function(b,c){if(!c)return!1;if($H.isArr(c))for(var a=0,e=c.length;a<e;a++){if(this.match(b,c[a]))return!0}else if(this.match(b,c))return!0;return!1},doConfig:function(){this.callSuper();this.cls||(this.cls=this.xtype.toLowerCase());this.extCls=this.getExtCls();this.icon&&this.add({xtype:"Icon",name:this.icon});
...
return b=$H.Validator.valid(c,b)}for(var c=this.children,a=0,e=c.length;a<e;a++)if(b=c[a].valid(),!b)return b;return!0}},{define:function(b,f){var d=$H.createClass();$H.inherit(d,f||c,null,null,{notCover:function(b){return"define"==b}});$H.getSingleton(a).registerType(b,d);return $C[b]=d}});return c});$Define("C.Panel","C.AbstractComponent",function(a){a=a.define("Panel");a.extend({tmpl:['<div><%=this.content||this.findHtml(">*")%></div>']});return a});
...
hide:function(){!1!=this.callSuper()&&(this.noMask||this.unmask(),this.notDestroy||this.destroy())},center:function(){var b=this.getEl(),c=this.width||b.width(),a=this.height||b.height(),e=document,c=((e.documentElement.offsetWidth||e.body.offsetWidth)-c)/2,a=((e.documentElement.clientHeight||e.body.clientHeight)-a)/2+(e.documentElement.scrollTop||e.body.scrollTop),a=10>a?window.screen.height/2-200:a;b.css({left:c+"px",top:a-(this.offsetTop||0)+"px"})},followEl:function(b){b=(b||this.parent.getEl()).position();
this.getEl().css(b)},mask:function(){c||(c=$('<div class="hui-mask" style="display:none;"></div>').appendTo(document.body));c.css("z-index",1E3*b+998);0==b&&c.show();b++},unmask:function(){b--;0==b?c.hide():c.css("z-index",1E3*(b-1)+998)}});return a});
...
$Define("C.Icon","C.AbstractComponent",function(a){a=a.define("Icon");a.extend({tmpl:'<span class=";<%if(this.isAlt){%>; hui-alt-icon;<%}%>; hui-icon-<%=this.name%>;<%if(!this.noBg){%>; hui-icon-bg;<%}%>"></span>'.split(";")});return a});
...
"</div>"],listeners:[{name:"click",handler:function(){this.showOptions()}}],doConfig:function(b){var c=this;c.callSuper();for(var a=b.options,f=!1,d=0,e=a.length;d<e;d++){var g=a[d];if(g.value==b.value){c.text=g.text;f=g.selected=!0;break}}f||delete c.value;c.add({itemClick:function(b){c.val(b.value)},width:c.width,items:a})},showOptions:function(){this.children[0].show()},val:function(b){if(b){if(this.value!=b){var c=this.children[0],a=c.find(">[value="+b+"]");0<a.length&&(this.trigger("change"),
...
el:".js-input",handler:function(){this.getEl().addClass("hui-focus")}},{name:"blur",el:".js-input",handler:function(){this.getEl().removeClass("hui-focus")}}],doConfig:function(){var b=this;b.callSuper();"search"==b.type?b.icon="search":"textarea"==b.type&&b.listeners.push({name:"input propertychange",el:".js-input",handler:function(){var c=b.findEl(".js-input"),a=c[0].scrollHeight;50<=a&&c.css("height",a)}});b.withClear&&b.add({xtype:"Button",radius:"big",icon:"delete",click:function(){this.parent.findEl("input").val("").focus()}})},
parseItem:function(b){"Icon"==b.xtype?this.hasIcon=!0:"Button"==b.xtype&&(this.hasBtn=!0)},val:function(b){var c=this.findEl("input,textarea");if(b)c.val(b);else return c.val()},focus:function(){this.findEl("input").focus()}});return a});
$Define("C.Label","C.AbstractComponent",function(a){a=a.define("Label");a.extend({tmpl:['<label class="','<%if(this.labelColor){%> hui-label-<%=this.labelColor%><%}%><%if(this.labelAlign){%> c-txt-<%=this.labelAlign%><%}%>" for="<%=this.forName%>">',"<%=this.text%>","</label>"]});return a});
...
void 0==this.underline&&(this.underline=!0);b.click&&void 0==this.hasArrow&&(this.hasArrow=!0)}});return a});$Define("C.Set","C.AbstractComponent",function(a){a=a.define("Set");a.extend({tmpl:'<div>;<h1 class="hui-set-title"><%=this.title%></h1>;<div class="hui-set-content">;<%=this.findHtml(">*")%>;</div>;</div>'.split(";")});return a});
...
d(b)}})}});c.extend({okTxt:"\u786e\u5b9a",cancelTxt:"\u53d6\u6d88",clickHide:!1,radius:"little",tmpl:'<div>;<%=this.findHtml(">[xrole=dialog-header]")%>;<div class="hui-dialog-body">;<%if(this.content){%><%=this.content%><%}else{%>;<div class="hui-body-content">;<h1 class="hui-content-title"><%=this.contentTitle%></h1>;<div class="hui-content-msg"><%=this.contentMsg%></div>;<%=this.findHtml(">[xrole=dialog-content]")%>;</div>;<%}%>;<%if(!this.noAction){%>;<div class="hui-body-action">;<%=this.findHtml(">[xrole=dialog-action]")%>;</div>;<%}%>;</div>;</div>'.split(";"),
...
"</div>"],selectItem:function(a,b){b=!1!=b;"dot"==this.markType?(a.selected=b,a.getEl().parent()[!1==b?"removeClass":"addClass"]("hui-item-mark")):"active"==this.markType?c.prototype.selectItem.call(this,a,b):a.selected=b}});return a});$Define("C.List",["C.AbstractComponent","C.ControlGroup"],function(a,b){var c=a.define("List",b);c.extend({cls:"list",tmpl:["<div>",'<div class="hui-list-item c-clear">',"</div>","</div>"]});return c});
